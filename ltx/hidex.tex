...
\begin{remark}
	...
	where
	\begin{equation}
	\Phi = \int_\mathcal{S}ds\phi(s)\phi^\top(s)
	\end{equation}
	This follows directly by substituting $f_t(s) = x_t^\top\phi(s)$ into the definition of the covariance function $Q(s,s')$ \cite{Scerri09}.
\end{remark}

Finally, using the field decomposition (\ref{eq:f decomp}) we can rewrite 
\begin{equation}
	\dist{f_t(s) - \hat{f}_t(s)}{Q^{-1}} = \dist{x_t - \hat{x}_t}{\Xi}
\end{equation}
where $\Xi = \int\int ds' ds ~ \phi(s')Q^{-1}(s',s)\phi(s)^\top$ allowing us to replace the infinite dimensional term in the likelihood with a finite dimensional term via the decomposition, giving
\begin{equation}
...
\begin{equation}
	\phi(s)^\top\hat{x}_t = \sum_{i=1}^{p}(\F_i \phi^\top)(s)x_{t-i} + \sum_{j=1}^{q}(\G_j\phi^\top)(s)u_{t-j}
\end{equation}
To isolate $\hat{x}_t$ on the left hand side, we pre-multiply both sides by $\phi(s)$, integrate over space, and pre-multiply both sides by the resulting matrix, giving
\begin{equation}
	\begin{split}
	\hat{x}_t = 
		\sum_{i=1}^{p} \Phi^{-1} \int_\mathcal{S}ds \phi(s)(\F_i \phi^\top)(s)x_{t-i} + \\
	\sum_{j=1}^{q} \Phi^{-1} \int_\mathcal{S}ds \phi(s)(\G_j\phi^\top)(s)u_{t-j}
	\end{split}
\end{equation}
...
\begin{remark}
	...
	\begin{equation}
		\label{eqn:arx}
		\hat{x}_t = \sum_{i=1}^{p} A_i^\top x_{t-i} +  \sum_{j=1}^{q} B_j^\top u_{t-j}
	\end{equation}
...
\end{equation}
then the `state' of the decomposed system can be thought of as the information required to predict the next field. With these definitions, and the recursive relationship on $\hat{x}$ defined above, the formation of $A$, $B$ and $C$ are trivial. The state transition matrix is defined by concatenating the $A_i$s defined in \ref{eqn:arxA}:
\begin{equation}
	A = \begin{bmatrix}
	A_1 & A_2 & \ldots & A_p \\
...
The equations \ref{eqn:ssmodel_hidden} and \ref{eqn:ssmodel_observations} that define the state space model simply recapitulate the model distributions \ref{eqn:fdist} and \ref{eqn:ydist}
