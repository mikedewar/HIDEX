...
\begin{remark}
	...
	\begin{equation}
		p(x_t\phi(s)) = \mathrm{GP}(\hat{x}\phi(s),Q(s,s))
	\end{equation}
	We can expand the left hand side trivially and, noting that we treat $\phi$ as a deterministic function:
	 \begin{equation}
		p(x_t|\phi(s))p(\phi(s)) = \mathrm{N}(\hat{x},\dist{\phi}{Q^{-1}})\delta(\phi(s))
	\end{equation}
	where $\delta$ is the Dirac measure and we use the property that any finite sample of a Gaussian Process is normally distributed. Here the covariance matrix is found somehow \todo{How to prove this is the cov?}
\end{remark}
