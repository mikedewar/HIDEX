...
We follow the standard derivation of the EM algorithm \cite{Ninness}. We first derive the maximum likelihood estimate of the parameters given distributions over the hidden field. We then show how to update the distributions over the hidden field given estimates of the parameters. 

The key insights here are:
\begin{itemize}
	\item we can represent the expected log likelihood function for the HIDEX($p,q$) model as a quadratic in the weights of the decomposed spatial mixing kernels
	\item given a set of parameters, generating distributions over the hidden field becomes a straightforward application of the Rauch-Tung-Streibel smoother \cite{Rauch}.
\end{itemize} 
...
\begin{equation}
	\E[\ell(\theta)] = \epsilon - \theta^\top \lambda + \theta^\top \Gamma \theta
\end{equation}
where we concatenate the unknown parameters into the vector $\theta = [a_1^\top ~ a_2^\top ~ \ldots ~ a_p^\top ~ b_1^\top ~ b_2^\top ~ \ldots ~ b_q^\top ~ c^\top]^\top$ and similarly the linear coefficient into the vetor vector $\lambda = [\lambda^{a\top}_1 ~ \lambda^{a\top}_2 ~ \ldots ~ \lambda^{a\top}_p ~ \lambda^{b\top}_1 ~ \lambda^{b\top}_2 ~ \ldots ~ \lambda^{b\top}_q ~ \lambda^{c\top}]^\top$. The matrix $\Gamma$ is formed by simply arranging all the $\Gamma$-variables in a large matrix. To see this, let
\begin{equation}
	\Gamma^a = \begin{bmatrix}
		\Gamma_{1,1}^a & \Gamma_{1,2}^a & \ldots & \Gamma_{1,q}^a \\
		\Gamma_{2,1}^a & \Gamma_{2,2}^a &  &  \\
		\vdots & & \ddots &  \\
		\Gamma_{q,1}^a &  &  & \Gamma_{q,q}^a \\
	\end{bmatrix}
\end{equation}
\begin{equation}
	\Gamma^b = \begin{bmatrix}
		\Gamma_{1,1}^b & \Gamma_{1,2}^b & \ldots & \Gamma_{1,q}^b \\
		\Gamma_{2,1}^b & \Gamma_{2,2}^b &  &  \\
		\vdots & & \ddots &  \\
		\Gamma_{q,1}^b &  &  & \Gamma_{q,q}^b \\
	\end{bmatrix}
\end{equation}
\begin{equation}
	\Gamma^{ab} = \begin{bmatrix}
		\Gamma_{1,1}^{ab} & \Gamma_{1,2}^{ab} & \ldots & \Gamma_{1,p}^{ab} \\
		\Gamma_{1,2}^{ab} & \Gamma_{2,2}^{ab} &  &  \\
		\vdots & & \ddots &  \\
		\Gamma_{1,q}^{ab} &  &  & \Gamma_{p,q}^{ab} \\
	\end{bmatrix}
\end{equation}
Given these matrices, we can write $\Gamma$ as 
\begin{equation}
	\Gamma = \begin{bmatrix}
		\Gamma_a & \Gamma^{ab} & 0_{n\times n} \\
		\Gamma^{ab\top} & \Gamma^{b} & 0_{n\times n} \\
		0_{n\times n} & 0_{n\times n} & \Gamma_c
	\end{bmatrix}
\end{equation}
