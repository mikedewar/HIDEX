...
\begin{definition}[HIDEX Model]
	...
	f_t(s)= \sum_{i=1}^{p} (\F f_{t-i})(s) + \sum_{j=1}^{q}(\G g_{t-j})(s)+e(s)
	\end{equation}
	\begin{equation}
	\label{eqn:hidex-obs}
...
where $R = \sigma I_m$,
\begin{equation}
	\hat{f}_t(s) =\sum_{i=1}^{p}( \F f_{t-i})(s) + \sum_{j=1}^{q}(\G g_{t-j})(s)
\end{equation}
and 
\begin{equation}
...
\end{equation}	 
where $\xvec_t \in \R^{n_x}$ is a vector of unknown weights, referred to as the state of the hidden field at time $t$, and $\uvec_t \in \R^{n_x}$ is a vector of known weights, referred to as the input at time $t$. Here, $\phi(s)$ is a vector of basis functions drawn from the spanning set thingy.
\end{assumption}
...
\begin{equation}
	H(s,s') = c^\top\psi(s,s')
\end{equation}
where $a_i, b_j, c \in \R^{n_y}$ are the parameters of the kernels $F_i, G_j$ and $H$ respectively. Here $\psi(s)$ is a vector of basis functions drawn from the spanning set thingy.
\end{assumption}
...
\end{equation}
which is convex in $\hat{\xvec}_t$ and $\hat{\yvec}_t$. Recall that 
\begin{equation}
	\hat{f}_t(s) = \sum_{i=1}^{p}( \F f_{t-i})(s) + \sum_{j=1}^{q}(\G g_{t-j})(s)
\end{equation}
By introducing the decomposition $g_t(s) = \phi(s)^\top\uvec_t$ this can be re-written
\begin{equation}
	\phi(s)^\top\hat{\xvec}_t = \sum_{i=1}^{p}(\F \phi)(s)\xvec_{t-i} + \sum_{j=1}^{q}(\G\phi)(s)\uvec_{t-j}
\end{equation}
By decomposing the spatial mixing kernels $F_i(s,s')$ and $G_j(s,s')$:
\begin{equation}
	F_i(s,s') = a_i^\top\psi(s,s')
\end{equation}
\begin{equation}
	G_j(s,s') = b_j^\top\psi(s,s')
\end{equation}
we can expand the integral operators $\mathbb{F}$ and $\mathbb{G}$, giving 
\begin{equation}
...
\end{equation}
where $\epsilon$ has been abused somewhat to include additional terms that do not depend on the unknown parameters $\theta$. We will deal with each of these terms in turn, using the decompositions to write them explicitly in terms of the unknown parameters.
